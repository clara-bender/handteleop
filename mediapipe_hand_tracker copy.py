...
class MediaPipeHandTracker(Node):
    ...
    def _get_first_hand(self, results, type):
        """Return landmarks for the first detected hand, or None."""
        if (type=="normalized" and 
            results.multi_hand_landmarks is not None and
            len(results.multi_hand_landmarks) > 0):
            return results.multi_hand_landmarks[0].landmark
        elif (type=="world" and
              results.multi_hand_world_landmarks is not None and
              len(results.multi_hand_world_landmarks) > 0):
            return results.multi_hand_world_landmarks[0].landmark
        return None
    ...
    def _synced_callback(self, color_msg: Image, depth_msg: Image,
        ...
        try:
            ...
            landmarks = self._get_first_hand(results,"normalized")
            world_landmarks = self._get_first_hand(results,"world")
            ...
            if detected:
                ...
                #     self.get_logger().info("Using RealSense Depth")
                ...
                #     cam.latest_world_landmarks = world_landmarks
                # else:
                # Depth fallback — use MediaPipe normalized coords
                self.get_logger().info("Using Fallback")
                action = self.transformer.landmarks_to_action(landmarks,world_landmarks)
                cam.latest_position = action['position']
                self.get_logger().info(f"y-pos: {action['position'][1]}")
                self.get_logger().info(f"hand y: {action['raw_hand_position'][0]}")
                cam.latest_yaw = yaw
                cam.latest_time = time.monotonic()
                cam.latest_landmarks = landmarks
                cam.latest_world_landmarks = world_landmarks
                ...
                cam.latest_world_landmarks = None

            if cam.latest_position is not None:
                self.get_logger().info(f'Latest position: {cam.latest_position}')
    ...
    def _fuse_and_publish(self, stamp, depth_image=None, img_w=0, img_h=0):
        ...
        if tracking_active:
            ...
            self.get_logger().info(f'Hand position: {fused_position})')

            # Publish gripper from primary camera's landmarks
            if primary_cam is not None and primary_cam.latest_landmarks is not None:
                gripper = self.transformer.get_gripper_from_pinch(primary_cam.latest_landmarks,primary_cam.latest_world_landmarks)
                gripper_msg = Float32()
